...
Program  PL0_compile (input, output);
{带有代码生成的PL0编译程序}
...
Begin
  writeln(file_output ,'****', ' ': cc-1, '↑', n : 2);{cc为当前行已读的字符数, n为错误号}
  err := err + 1{错误数err加1}
End {error};
Procedure getsym;
...
Begin
   If cc = ll Then {如果cc指向行末}
    Begin
      If eof(file_input) Then{如果已到文件尾}
        Begin
          write(file_output,'PROGRAM INCOMPLETE');
          close(file_input);
          close(file_output);
          exit;
          //goto 99
      ...
      ll := 0;{读新的一行}
      cc := 0;
      write(file_output,cx : 5, ' ');{cx : 5位数}
      While not eoln(file_input) Do{不是行末}
        Begin
          ll := ll + 1;
          read(file_input,ch);
...
Begin
  If cx > cxmax Then
    Begin
      write(file_output,'PROGRAM TOO LONG');
      close(file_input);
      close(file_output);
      //goto 99
      exit;
...
Begin  {列出本程序体生成的代码}
  For i := cx0 To cx-1 Do
    With code[i] Do
      writeln(file_output, i:5, mnemonic[f] : 7, l : 3, a : 5)
End {listcode};
...
Begin {statement}
  If sym = ident Then
    Begin
      i := position(id);
      If i = 0 Then error(11) {read problem}
      Else
...
End {base};
Begin
  writeln(file_output,'START PL/0');
  t := 0;
  b := 1;
  p := 0;
  s[1] := 0;
